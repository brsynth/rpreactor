...
def _create_db_from_retrorules_v1_0_5(path_retrosmarts_tsv, db_path, with_hs, with_stereo):
    ...
    with open(path_retrosmarts_tsv) as hdl:
        for row in csv.DictReader(hdl, delimiter='\t'):
            ...
            usage = row["Rule_usage"]  # -1, 0, 1 ==> retro, both, forward
            if usage == "both":
                usage = 0
            elif usage == "retro":
                usage = -1
                ...
                raise ValueError(f"Found an unexpected usage for rule {rid}: {usage}")
            if rid not in rules:
                # warning: keys must match database schema
                rules[rid] = {'rd_rule': rsmarts, 'diameter': diameter, 'usage': usage}
            else:
