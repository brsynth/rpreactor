...
def convert_depiction(idepic, itype='smiles', otype={'inchikey'}):
    ...
    # Import (if needed)
    if itype == 'smiles':
        rdmol = Chem.MolFromSmiles(idepic, sanitize=True)
    elif itype == 'inchi':
        rdmol = Chem.MolFromInchi(idepic, sanitize=True)
    else:
    ...
    for item in otype:
        if item == 'smiles':
            odepic[item] = Chem.MolToSmiles(rdmol)  # MolToSmiles is tricky, one mays want to check the possible options..
        elif item == 'inchi':
            odepic[item] = Chem.MolToInchi(rdmol)
        elif item == 'inchikey':
